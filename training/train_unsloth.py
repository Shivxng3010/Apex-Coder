...
def train_apex_model(
    ...
    if adapt_from and os.path.exists(adapt_from):
        print(f"[*] Initializing from prior LoRA checkpoint: {adapt_from}", flush=True)
        model = PeftModel.from_pretrained(model, adapt_from, is_trainable=True)
    elif resume_checkpoint and os.path.exists(resume_checkpoint):
    ...
    global_step = 0
    if resume_checkpoint and not adapt_from:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume", "--resume-checkpoint", dest="resume", type=str, default=None)
    parser.add_argument("--adapt-from", type=str, default=None, help="Prior LoRA checkpoint to initialize weights from for targeted fine-tuning")
